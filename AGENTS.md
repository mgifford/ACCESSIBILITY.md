...
- [Accessibility Finding Tracking](examples/ACCESSIBILITY_FINDING_TRACKING.md) — canonical model for tracker IDs, scan request/run IDs, occurrence and pattern fingerprints, legacy identifier migration, and the actionability policy distinguishing a valid intake report from a finding ready for remediation (active remediation queue vs. investigation queue vs. observation history, `not_observed` vs. `not_tested`). Changes to actionability or triage policy belong here and in [Accessibility Bug Reporting](examples/ACCESSIBILITY_BUG_REPORTING_BEST_PRACTICES.md); they must not change a frozen fingerprint profile, the schema's lifecycle enum, or a migration profile.
...
- [Accessibility Finding Schema](examples/schemas/README.md) — versioned JSON Schema (`schema_version: "2.0"`) for the machine-readable finding record. Validate any change to a finding record's shape, an example, or the schema itself with `cd examples/schemas && npm ci && npm run validate`. Preserve support for manual and user-reported findings with no fingerprint, tracker ID, WCAG mapping, or technical locator. Do not change a frozen fingerprint profile while doing schema work; run both `examples/fingerprints` and `examples/schemas` checks before treating schema-related changes as complete.
- [Accessibility Migration Profiles](examples/migrations/ACCESSIBILITY_MIGRATION_PROFILES.md) — verified `drupal-core` and `open-scans` legacy identifier formats and migration requirements. Before adding or changing a repository-specific claim here, verify it against that repository's current source; do not restate an old claim from memory or from this file's git history as if it were still current. This guide documents but does not implement a migration; do not use it as authorization to modify `drupal-core`, `open-scans`, or `accessibility-skills`.
